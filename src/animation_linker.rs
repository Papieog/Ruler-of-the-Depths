...
#[derive(Component)]
pub struct AnimationEntityPlayerLink(pub Entity);
...
fn get_second_top_parent(mut curr_entity: Entity, parent_query: &Query<&Parent>) -> Entity {
    //Loop up all the way to the top parent
...
pub fn link_animations_parent(
    player_query: Query<Entity, Added<AnimationPlayer>>,
    parent_query: Query<&Parent>,
    animations_entity_link_query: Query<&AnimationEntityPlayerLink>,
    mut commands: Commands,
) {
    // Get all the Animation players which can be deep and hidden in the heirachy
    for entity in player_query.iter() {
        let top_entity = get_second_top_parent(entity, &parent_query);
        ...
        if animations_entity_link_query.get(top_entity).is_ok() {
            ...
            commands
                .entity(top_entity)
                .insert(AnimationEntityPlayerLink(entity.clone()));
        }
    }
}