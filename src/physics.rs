...
impl Plugin for PhysicsPlugin {
    fn build(&self, app: &mut App) {
        ...
        .add_systems(Update, collision_gravity_drag)
        ;
    }
}
...
pub fn collision(
    ...
    while let Some([(mut physics, transform, enemy), (mut other_physics, other_transform, other_enemy)]) = combinations.fetch_next() {
        if transform.translation.distance(other_transform.translation) < (physics.collider + other_physics.collider)*2.{
